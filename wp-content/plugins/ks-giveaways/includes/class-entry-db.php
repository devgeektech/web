...
class KS_Entry_DB extends KS_Database_Table
{
  ...
  KEY entry_contestant_referral (contestant_id,referral_id)
    ...
    public static function draw($contest_id, $overwrite_id = null, $exclude = null)
    {
        ...
        if ($mode === 'confirmed') {
            $confirmed = " AND {$contestant_table}.`status` = 'confirmed' ";
        } else {
            $confirmed = "";
        }

        $query = trim($wpdb->prepare("
          SELECT COUNT({$table}.`ID`) FROM {$table} JOIN {$contestant_table} ON {$contestant_table}.`ID` = {$table}.`contestant_id` WHERE {$contestant_table}.`contest_id` = %d {$confirmed}
        ...
        $query = trim($wpdb->prepare("
          SELECT {$contestant_table}.* FROM {$table} JOIN {$contestant_table} ON {$contestant_table}.`ID` = {$table}.`contestant_id` WHERE {$contestant_table}.`contest_id` = %d {$confirmed}
          AND {$contestant_table}.`ID` NOT IN (SELECT `contestant_id` FROM {$winner_table} WHERE `contest_id` = %d)
          LIMIT {$offset},1
        ", $contest_id, $contest_id));
