...
class Setlr_Payment {
    ...
    public static function render_payment_form( $customer_id, $amount, $currency, $project_id ) {
        ...
        ?>
            <form id="checkout" method="post" action="<?php echo admin_url('admin.php'); ?>">
                ...
                <input type="submit" value="<?php printf( __( 'Pay %s', 'pdcrequest' ), $currency_symbol . $amount ); ?>">
    ...
    public static function enqueue_scripts() {
        wp_register_script( 'braintree', 'https://js.braintreegateway.com/v2/braintree.js', array(), 'v2', true );
        wp_register_script( 'setlr-braintree-payment', plugins_url('pdc-requestmgt/assets/js/setlr-braintree-payment.js'), array( 'jquery', 'braintree'), '0.9', true );
        wp_localize_script( 'setlr-braintree-payment', 'pdcrequest', array( 'ajaxurl' => admin_url( 'admin-ajax.php' ) ) );
        
        if ( is_page('project-quote') || is_page( 'add-project-profile' ) || is_page( 'add-a-new-project' )) :
    ...
    public static function do_payment() {
        ...
        self::redirect_after_payment( $result->transaction->processorResponseCode, $project_id );
    }
    
    
    public static function redirect_after_payment( $payment_result, $project_id ) {
