...
class lang {
    ...
    public static function render_lang_select( $type = 'from', $default = array() ) {
        ...
        foreach ( $langs as $key => $value ) :
            foreach( $value as $code => $name ) :
                $default_value = ( isset( $default[$default_key])) ? $default[$default_key] : '';
               // write_log( 'default_value for ' . $select_id . ' = ' . $default_value);
                if ( isset( $default[$default_key])) :
                    $selected = selected( $default[$default_key], $code, false );
                else :
                    $selected = '';
                endif;
    ...
    /**
     * calculates confidence score as percentage
     * @param array $array
    ...
    public static function request_locales_form( $request_id ) {
        ...
        if ( empty( $request_lang ) ) :
            $request_lang = get_post_meta( $request_id, 'to-lang', true );
        endif;
    ...
    /* -------- Removed NOT Important option for "Helper" user role. ------ */
