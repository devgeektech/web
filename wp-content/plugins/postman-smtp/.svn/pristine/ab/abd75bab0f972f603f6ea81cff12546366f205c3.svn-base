 ...
 *        
 */
abstract class PostmanAbstractOptions implements PostmanOptionsInterface {
	...
	const SUBSITES_ACCOUNT_UNDER_NETWORK_CONTROL = 'mu_account';
	const SUBSITES_MESSAGE_UNDER_NETWORK_CONTROL = 'mu_message';
	...
	const DEFAULT_LOG_LEVEL = PostmanLogger::WARN_INT;
	...
	public function isSubsiteAccountSettingsUnderNetworkControl() {
		if ($this->isNew ())
			return false;
		if (isset ( $this->options [PostmanOptions::SUBSITES_ACCOUNT_UNDER_NETWORK_CONTROL] ))
			return $this->options [PostmanOptions::SUBSITES_ACCOUNT_UNDER_NETWORK_CONTROL];
	}
	public function isSubsiteMessageSettingsUnderNetworkControl() {
		if ($this->isNew ())
			return false;
		if (isset ( $this->options [PostmanOptions::SUBSITES_MESSAGE_UNDER_NETWORK_CONTROL] ))
			return $this->options [PostmanOptions::SUBSITES_MESSAGE_UNDER_NETWORK_CONTROL];
	}
...
if (! class_exists ( "PostmanOptions" )) {
	 ...
	 */
	class PostmanOptions extends PostmanAbstractOptions {
		// singleton instance
		...
		public function save() {
			if (function_exists ( 'is_network_admin' ) && is_network_admin()) {
				update_site_option ( PostmanOptions::POSTMAN_OPTIONS, $this->options );
			} else {
		...
		public function load() {
			if (function_exists ( 'is_network_admin' ) && is_network_admin()) {
				$this->options = get_site_option ( PostmanOptions::POSTMAN_OPTIONS );
			} else {
 ...
 * Make sure these emails are permitted (see http://en.wikipedia.org/wiki/E-mail_address#Internationalization):
 */
class PostmanNetworkOptions extends PostmanAbstractOptions {
	// singleton instance
	public static function getInstance() {
		static $inst = null;
		if ($inst === null) {
			$inst = new PostmanNetworkOptions ();
		}
		return $inst;
	}
	//
