...
if (! class_exists ( "PostmanMessage" )) {
	...
	class PostmanMessage {
		 ...
		 */
		function __construct($resendData = null) {
			...
			if ($resendData != null) {
				$this->importResendData ( $resendData );
			}
		}
		
		/**
		 * Formats the contents of the object as a simple array for peristence
		 *
		 * @return multitype:multitype: NULL
		 */
		public function getResendData() {
			$data = array ();
			if (isset ( $this->from )) {
				$data ['from_email'] = $this->getFromAddress ()->getEmail ();
				$data ['from_name'] = $this->getFromAddress ()->getName ();
				$data ['from_formatted'] = $this->getFromAddress ()->format ();
			}
			if (isset ( $this->replyTo )) {
				$data ['reply_to_email'] = $this->getReplyTo ()->getEmail ();
				$data ['reply_to_name'] = $this->getReplyTo ()->getName ();
				$data ['reply_to_formatted'] = $this->getFromAddress ()->format ();
			}
			$data ['to_flat'] = $this->flattenEmails ( $this->getToRecipients () );
			$data ['to_recipients'] = array ();
			foreach ( $this->getToRecipients () as $recipient ) {
				$recipientArray = array (
						'email' => $recipient->getEmail (),
						'name' => $recipient->getName () 
				);
				array_push ( $data ['to_recipients'], $recipientArray );
			}
			...
			foreach ( $this->getCcRecipients () as $recipient ) {
				...
				array_push ( $data ['cc_recipients'], $recipientArray );
			}
			...
			foreach ( $this->getBccRecipients () as $recipient ) {
				...
				array_push ( $data ['bcc_recipients'], $recipientArray );
			}
			$data ['subject'] = $this->getSubject ();
			$data ['body'] = $this->getBody ();
			$data ['body_text'] = $this->getBodyTextPart ();
			$data ['body_html'] = $this->getBodyHtmlPart ();
			$data ['headers'] = $this->getHeaders ();
			$data ['content_type'] = $this->getContentType ();
		...
		/**
		 */
		public function importResendData($data) {
			$this->logger->trace ( $data );
			if (isset ( $data ['from_email'] )) {
				$this->from = new PostmanEmailAddress ( $data ['from_email'], $data ['from_name'] );
			}
			if (isset ( $data ['reply_to_email'] )) {
				$this->replyTo = new PostmanEmailAddress ( $data ['reply_to_email'], $data ['reply_to_name'] );
			}
			foreach ( $data ['to_recipients'] as $recipient ) {
				$address = new PostmanEmailAddress ( $recipient ['email'], $recipient ['name'] );
				array_push ( $this->toRecipients, $address );
			}
			foreach ( $data ['cc_recipients'] as $recipient ) {
				$address = new PostmanEmailAddress ( $recipient ['email'], $recipient ['name'] );
				array_push ( $this->ccRecipients, $address );
			}
			foreach ( $data ['bcc_recipients'] as $recipient ) {
				$address = new PostmanEmailAddress ( $recipient ['email'], $recipient ['name'] );
				array_push ( $this->bccRecipients, $address );
			}
			$this->subject = $data ['subject'];
			$this->body = $data ['body'];
			$this->bodyTextPart = $data ['body_text'];
			$this->bodyHtmlPart = $data ['body_html'];
		 ...
		 */
		public function applyFilters(PostmanModuleTransport $transport) {
			...
			$forcedEmailAddress = $transport->getFromEmailAddress();
			if ($transport->isSenderEmailOverridePrevented () && $this->getFromAddress ()->getEmail () !== $forcedEmailAddress) {
			...
			$forcedEmailName = $transport->getFromName();
			if ($transport->isSenderNameOverridePrevented () && $this->getFromAddress ()->getName () !== $forcedEmailName) {
