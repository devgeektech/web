...
class PostmanInputSanitizer {
	...
	public function sanitize($input) {
		if ($this->logger->isInfo ()) {
			$this->logger->info ( "Sanitizing data before storage" );
		}
		...
		$this->sanitizeString ( 'Subsite Account', PostmanOptions::SUBSITES_ACCOUNT_UNDER_NETWORK_CONTROL, $input, $new_input );
		$this->sanitizeString ( 'Subsite Message', PostmanOptions::SUBSITES_MESSAGE_UNDER_NETWORK_CONTROL, $input, $new_input );
	...
	private function sanitizeLogMax($desc, $key, $input, &$new_input) {
		if (isset ( $input [$key] )) {
			...
			if ($value <= 0) {
				...
				$h = PostmanMessageHandler::getInstance ();
