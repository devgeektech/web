...
if (! class_exists ( "PostmanZendMailEngine" )) {
	...
	class PostmanZendMailEngine implements PostmanMailEngine {
		...
		public function send(PostmanMessage $message) {
			...
			$mail->addHeader ( 'Sender', $this->transport->getEnvelopeFromEmailAddress(), false );
			...
			try {
				...
				if (method_exists($zendTransport, 'getConnection') && $zendTransport->getConnection () && $zendTransport->getConnection () && ! PostmanUtils::isEmpty ( $zendTransport->getConnection ()->getLog () )) {
				...
				// finally not supported??
				if (method_exists($zendTransport, 'getConnection') && $zendTransport->getConnection () && ! PostmanUtils::isEmpty ( $zendTransport->getConnection ()->getLog () )) {
					$this->transcript = $zendTransport->getConnection ()->getLog ();
				} else if (method_exists ( $zendTransport, 'getTranscript' ) && ! PostmanUtils::isEmpty ( $zendTransport->getTranscript () )) {
					// then use the API response
					$this->transcript = $zendTransport->getTranscript ();
