...
class PostmanSendTestEmailController {
	...
	public function __construct($rootPluginFilenameAndPath) {
		...
		PostmanUtils::registerAdminMenu ( $this, 'add_test_email_menu' );
		PostmanUtils::registerNetworkAdminMenu ( $this, 'add_network_test_email_menu' );
		...
		add_action ( 'postman_get_send_test_email_url', array (
				$this,
				'on_postman_get_send_test_email_url' 
		) );
	}
	 ...
	 * @return string
	 */
	public function on_postman_get_send_test_email_url() {
		if (PostmanUtils::isPageNetworkAdmin ()) {
			return 'admin.php?page=' . PostmanSendTestEmailController::EMAIL_TEST_SLUG;
		} else {
			return 'tools.php?page=' . PostmanSendTestEmailController::EMAIL_TEST_SLUG;
		}
	}
	...
	/**
	 */
	public function add_test_email_menu() {
		$this->addEmailTestSubmenu ( false );
	}
	
	/**
	 */
	public function add_network_test_email_menu() {
		$this->addEmailTestSubmenu ( true );
	}
	
	/**
	 * Register the Email Test screen
	 */
	public function addEmailTestSubmenu($network) {
		$menuName = __ ( 'Send a Test Email', Postman::TEXT_DOMAIN );
		$pageTitle = sprintf ( '%s - %s', __ ( 'Postman SMTP', Postman::TEXT_DOMAIN ), $menuName );
		$uniqueId = PostmanSendTestEmailController::EMAIL_TEST_SLUG;
		$pageOptions = array (
				$this,
