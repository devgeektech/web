...
class PostmanConnectivityTestController {
	...
	public function __construct($rootPluginFilenameAndPath) {
		...
		PostmanUtils::registerAdminMenu ( $this, 'add_port_test_menu' );
		PostmanUtils::registerNetworkAdminMenu ( $this, 'add_network_port_test_menu' );
		...
		add_action ( 'postman_get_connectivity_test_url', array (
				$this,
				'on_postman_get_connectivity_test_url' 
		) );
	}
	...
	public function on_postman_get_connectivity_test_url() {
		if (PostmanUtils::isPageNetworkAdmin ()) {
			return 'admin.php?page=' . PostmanConnectivityTestController::PORT_TEST_SLUG;
		} else {
			return 'tools.php?page=' . PostmanConnectivityTestController::PORT_TEST_SLUG;
		}
	}
	...
	/**
	 */
	public function add_port_test_menu() {
		$this->addPortTestSubmenu ( false );
	}
	
	/**
	 */
	public function add_network_port_test_menu() {
		$this->addPortTestSubmenu ( true );
	}
	
	/**
	 * Register the Email Test screen
	 */
	private function addPortTestSubmenu($networkMode) {
		$menuName = __ ( 'SMTP Server Connectivity Test', Postman::TEXT_DOMAIN );
		$pageTitle = sprintf ( '%s - %s', __ ( 'Postman SMTP', Postman::TEXT_DOMAIN ), $menuName );
		$uniqueId = PostmanConnectivityTestController::PORT_TEST_SLUG;
		$pageOptions = array (
				$this,
	...
	public function outputPortTestContent() {
		...
		PostmanViewController::outputChildPageHeader ( __ ( 'SMTP Server Connectivity Test', Postman::TEXT_DOMAIN ) );
		
		print '<p>';
		print __ ( 'This connectivity test determines which well-known services and ports are available for Postman to use.', Postman::TEXT_DOMAIN );
		...
		print sprintf ( '<tr><th rowspan="2">%s</th><th rowspan="2">%s</th><th rowspan="2">%s</th><th rowspan="2">%s</th><th rowspan="2">%s</th><th colspan="5">%s</th></tr>', __ ( 'Transport', Postman::TEXT_DOMAIN ), _x ( 'Socket', 'A socket is the network term for host and port together', Postman::TEXT_DOMAIN ), __ ( 'Status', Postman::TEXT_DOMAIN ), __ ( 'Service Available', Postman::TEXT_DOMAIN ), __ ( 'Server ID', Postman::TEXT_DOMAIN ), __ ( 'Authentication', Postman::TEXT_DOMAIN ) );
