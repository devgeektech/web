...
if (! class_exists ( "PostmanWpMail" )) {
	...
	class PostmanWpMail {
		...
		public function init() {
			...
			require_once (realpath ( dirname ( __FILE__ ) . '/../Postman-Email-Log/PostmanEmailLogService.php' ));
			require_once 'PostmanMailEngine.php';
			require_once realpath ( dirname ( __FILE__ ) . '/../Postman-Auth/PostmanAuthenticationManagerFactory.php' );
			require_once realpath ( dirname ( __FILE__ ) . '/../PostmanState.php' );
		 ...
		 * @return boolean
		 */
		public function send($to, $subject, $body, $headers = '', $attachments = array()) {
			
			// initialize for sending
			$this->init ();
			
			// build the message
			$postmanMessage = $this->applyWpMailFilter ( $to, $subject, $body, $headers, $attachments );
			...
			$postmanMessage->applyFilters ( $transport );
			
			// create the body parts (if they are both missing)
			// do this AFTER applying the filters
			if ($postmanMessage->isBodyPartsEmpty ()) {
				$postmanMessage->createBodyParts ();
			}
			
			// send the message and return the result
			return $this->sendMessage ( $postmanMessage );
		}
		 ...
		 * @param unknown $attachments        	
		 */
		private function applyWpMailFilter($to, $subject, $message, $headers, $attachments) {
		...
		public function createNewMessage() {
			...
			$transport = PostmanTransportRegistry::getInstance ()->getActiveTransport ();
			$emailAddress = $transport->getFromEmailAddress ();
		...
		/**
		 */
		private function getDefaultFromName() {
			return 'WordPress';
		}
		 ...
		 * @return boolean
		 */
		public function sendMessage(PostmanMessage $message) {
			
			// build the email log entry
			$resendData = $message->getResendData ();
			...
			try {
				...
				if ($options->getRunMode () == PostmanOptions::RUN_MODE_PRODUCTION || $options->getRunMode () == PostmanOptions::RUN_MODE_LOG_ONLY) {
					...
					PostmanEmailLogService::getInstance ()->writeSuccessLog ( $resendData, $engine->getTranscript (), $transport );
				}
				...
				if ($options->getRunMode () == PostmanOptions::RUN_MODE_PRODUCTION || $options->getRunMode () == PostmanOptions::RUN_MODE_LOG_ONLY) {
					...
					PostmanEmailLogService::getInstance ()->writeFailureLog ( $resendData, $engine->getTranscript (), $transport, $e->getMessage () );
				}
		...
		private function postSend(PostmanMailEngine $engine, $startTime, PostmanOptions $options, PostmanModuleTransport $transport) {
			...
			if ($this->logger->isInfo ()) {
				$this->logger->info ( sprintf ( 'Message %d accepted for delivery in %d miliseconds', PostmanState::getInstance ()->getSuccessfulDeliveries () + 1, $this->totalTime ) );
			}
		}
