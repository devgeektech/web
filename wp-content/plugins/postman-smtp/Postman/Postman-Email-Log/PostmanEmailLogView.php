...
class PostmanEmailLogView extends WP_List_Table {
	...
	function prepare_items() {
		...
		foreach ( $posts as $post ) {
			...
			if ($resendData != null) {
				$showTranscript = ! empty ( $resendData ['transcript'] );
				$showResend = true;
			} else {
				$meta_values = get_post_meta ( $post->ID );
				$showTranscript = ! empty ( $meta_values ['session_transcript'] [0] );
				$showResend = ! (empty ( $meta_values ['original_to'] [0] ) && empty ( $meta_values ['originalHeaders'] [0] ));
			}
			$date = $post->post_date;
